...
import { CodexAuthJson } from "../electron/services/codexProfileMirror.js";
...
describe("UsageService", () => {
  ...
  it("returns unavailable when the endpoint does not expose Codex limits", async () => {
    ...
    const fetchImpl: typeof fetch = vi.fn(async () => Response.json({ object: "page", data: [] }));
    const service = new UsageService({ fetchImpl });
  ...
  it("calls wham usage endpoint with bearer token and account id headers", async () => {
    ...
    // SAFETY: verified init is defined before asserting headers
  ...
  it("handles network timeout without crashing", async () => {
    await writeAuth({ tokens: { access_token: "access-test", account_id: "acct_123" } });
    const fetchImpl: typeof fetch = vi.fn((_url: string | URL | Request, init?: RequestInit) => new Promise<Response>((_resolve, reject) => {
  ...
  it.each([401, 404])("treats HTTP %i as unavailable", async (status) => {
    await writeAuth({ tokens: { access_token: "access-test", account_id: "acct_123" } });
    const fetchImpl: typeof fetch = vi.fn(async () => new Response("nope", { status }));
    const service = new UsageService({ fetchImpl });
  ...
  it("maps HTTP 403 to token expired", async () => {
    await writeAuth({ tokens: { access_token: "access-test", account_id: "acct_123" } });
    const fetchImpl: typeof fetch = vi.fn(async () => new Response("forbidden", { status: 403 }));
    const service = new UsageService({ fetchImpl });
...
async function writeAuth(content: CodexAuthJson) {
